...
import classe.medico.Medico;
import classe.medico.Procedimento;
import classe.paciente.Consulta;
import classe.paciente.Paciente;

import static classe.metodos.Validacoes.*;
...
public class MetodosGestaoHospitalar {
    ...
    public void adicionarNovoPaciente(Scanner scanner) {
        ...
        Paciente paciente = new Paciente.Builder(nome, idade)
                .cpf(cpf)
                .nomeDaRua(nomeRua)
                .numeroDaCasa(numeroCasa)
                .cep(cep)
                .sintomasDoPaciente(sintomas)
                .telefone(telefone)
                .protocolo(protocolo)
                .build();
    ...
    public void adicionarNovoMedico(Scanner scanner) {
        String cpf = Validacoes.validarCpf("Digite o CPF do médico: ", scanner);
    ...
    public void pesquisarMedicoPorCrm(Scanner scanner) {
        if (medicos.isEmpty()) {
        ...
        for (Medico medico : medicos) {
            if (medico.getCrm().equals(crm)) {
                String[] especialidades = {"Pediatria", "Cirurgia Geral", "Ginecologia e Obstetrícia", "Anestesiologia", "Ortopedia e Traumatologia", "Medicina do Trabalho", "Cardiologia"};
    ...
    public void excluirPacientePeloCpf(Scanner scanner) {
        String cpf = validarCpf("Digite o CPF do paciente que deseja excluir: ", scanner);
    ...
    public void excluirMedicoPorCrm(Scanner scanner) {
        String crm = Validacoes.validarCrm("Digite o CRM do médico (formato: CRM/XX123456): ");
    ...
    public void gerenciarConsultas(Scanner scanner) {
        ...
        while (true) {
            ...
            if (opcao == 0) {
                ...
                listarConsultas(scanner, especialidadeValores);
            } else if (opcao == 2) {
                agendarNovaConsulta(scanner, especialidadeValores);
            } else if (opcao == 3) {
    ...
    private void listarConsultas(Scanner scanner, Map<Integer, String> especialidadeValores) {
    ...
    private void agendarNovaConsulta(Scanner scanner, Map<Integer, String> especialidadeValores) {
        String cpfPaciente = Validacoes.validarCpf("Digite o CPF do paciente: ", scanner);
        boolean pacienteEncontrado = false;
        for (Paciente paciente : pacientes) {
            if (paciente.getCpf().equals(cpfPaciente)) {
